...
def main():
    ...
    for x in range (0, len(states)):
        if states[x] == state:
            lat1 = float(states[x+1])
            long1 = float(states[x+2])
            lat2 = float(states[x+3])
            long2 = float(states[x+4])

    locationCoord = [long1,lat1,long2,lat2]


    PoliticalTweetStream('republican', ['republican', 'Jeb Bush', 'Donald Trump', 'Ted Cruz', 'Chris Christie', 'Marco Rubio'], state, locationCoord, tweetCt)
    #democrat run
